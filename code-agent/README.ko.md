...
기본 ChatGPT/Codex 데스크톱 앱은 `~/.codex` 방식입니다. `sam-codex app`은
별도 SAM-Codex 앱을 만들거나 기존 데스크톱 앱의 provider를 안전하게
전환하는 명령이 아닙니다. SAM의 전용·권장 경로는 `sam-codex` 터미널
명령과 그 터미널 바로가기입니다.

Windows의 선택적 전환기는 기존 데스크톱 앱의 `~/.codex` 설정을 SAM으로
바꾸는 일시 전환 기능입니다. 두 방식을 동시에 제공하지 않으며, 일반
프로필로 되돌리는 복원 명령은 Windows 가이드에 있습니다.
