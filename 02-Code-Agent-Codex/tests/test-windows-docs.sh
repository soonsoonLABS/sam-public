...
for item in (
    ...
    "Invoke-RestMethod",
    "$env:SAM_API_KEY",
    "https://sam.soonsoon.ai/v2/codex/models",
    "https://sam.soonsoon.ai/v2/codex",
    "https://sam.soonsoon.ai/mcp",
    "sam-codex mcp list",
    "SAM-CODEX-OK",
    "official `codex`",
):
    assert item in combined, f"missing Windows Codex documentation contract: {item}"
...
for forbidden in (
    ...
    assert forbidden not in combined, f"secret-shaped content: {forbidden}"
...
print("PASS: Windows Codex docs, links, fences, PowerShell syntax, and secret hygiene")
PY
