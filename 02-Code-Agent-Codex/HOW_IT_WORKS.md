...
Codex 버전은 고정된 허용 목록으로 제한하지 않습니다. wrapper는 매 실행 시
설치된 semantic version을 읽어 discovery 요청과 cache envelope에 전달하고,
응답의 version이 현재 CLI와 일치하는지 확인합니다. catalog 형식이 바뀌거나
SAM이 허용하지 않은 모델이 반환되면 실행을 중단합니다.
