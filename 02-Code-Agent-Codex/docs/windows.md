...
설치 저장소 없이 직접 설정하거나 전환·복원 원리를 먼저 확인하려면
[Windows 수동 설정 가이드](../MANUAL_SETUP_WINDOWS.md)를 사용하세요. macOS
수동 설정은 [별도 문서](../MANUAL_SETUP.md)에 있습니다. 이 문서는 자동 설치
경로입니다.
...
Git 저장소를 내려받기 어려운 환경에서는 [Windows 수동 설정](../MANUAL_SETUP_WINDOWS.md)의
`sam-codex` wrapper 직접 생성 단계를 사용하세요.
